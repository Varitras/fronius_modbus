...
- The production limit, whether it is reached, and the battery converter's charge and discharge limits follow the same rule, since older firmware lacks these fields. Such sensors an entry already has from an earlier version stay; if they only ever show unknown, disable them.
