...
- An entry set up with an IPv4 address follows the inverter to a new address it announces: the host, the entry's title and the stored Web API token move with it. It does so only while the inverter no longer answers at the address the entry has: an answer proves that address, and the serial number in an announcement is no secret. An entry set up with a host name is left as it is, since the name finds the new address itself. While the inverter is unreachable, anyone able to send mDNS in your network could still announce another address for its serial number; if that is a concern, set the entry up with a host name.
